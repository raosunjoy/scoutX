...
async function fetchFootballData(token: string) {
  try {
    ...
    if (response.status === 429) {
      console.warn('Wyscout rate limit exceeded, retrying after delay...');
      await new Promise(resolve => setTimeout(resolve, 60000)); // Wait 1 minute
      return fetchFootballData(token); // Retry
    }

    return response.data.items.map((player: any) => ({
      sport: 'football',
      stats: { passingAccuracy: player.stats.passingAccuracy || Math.random() * 40 + 60 },
      success: player.stats.matchesPlayed > 20 ? 1 : 0,
    }));
  } catch (error) {
    console.error('Wyscout API error:', {
...
async function fetchCricketData(token: string) {
  try {
    ...
    if (response.status === 429) {
      console.warn('Catapult rate limit exceeded, retrying after delay...');
      await new Promise(resolve => setTimeout(resolve, 60000));
      return fetchCricketData(token);
    }

    return response.data.items.map((athlete: any) => ({
      sport: 'cricket',
      stats: { battingAverage: athlete.metrics.battingAverage || Math.random() * 50 + 20 },
      success: athlete.metrics.inningsPlayed > 10 ? 1 : 0,
    }));
  } catch (error) {
    console.error('Catapult API error (cricket):', {
      message: error.message,
      status: error.response?.status,
      data: error.response?.data,
    });
...
async function fetchBasketballData(token: string) {
  try {
    ...
    if (response.status === 429) {
      console.warn('Catapult rate limit exceeded, retrying after delay...');
      await new Promise(resolve => setTimeout(resolve, 60000));
      return fetchBasketballData(token);
    }
    ...
    return response.data.items.map((athlete: any) => ({
      ...
      success: athlete.metrics.gamesPlayed > 15 ? 1 : 0,
    }));
  } catch (error) {
    console.error('Catapult API error (basketball):', {
      message: error.message,
      status: error.response?.status,
      data: error.response?.data,
    });
