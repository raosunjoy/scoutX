...
const FanScreen = ({ navigation }) => {
  ...
  const renderTrends = (playerName) => {
    ...
    const datasets = [];
    const colors = ['#FF6347', '#4682B4', '#32CD32']; // Colors for different academies
    let colorIndex = 0;

    // For each academy, create datasets for stats and Success Score
    Object.entries(trends).forEach(([academy, data]) => {
      const color = colors[colorIndex % colors.length];
      colorIndex++;

      // Add dataset for each stat
      Object.entries(data.stats).forEach(([stat, values]) => {
        datasets.push({
          label: `${academy} - ${stat}`,
          data: values,
          borderColor: color,
          borderDash: [5, 5], // Dashed line for stats
          fill: false,
          yAxisID: 'y',
        });
      });

      // Add dataset for Success Score
      datasets.push({
    ...
    // Merge timestamps across all academies
